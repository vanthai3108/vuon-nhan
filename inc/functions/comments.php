...
if ( ! function_exists( 'farmart_comment' ) ) :
	function farmart_comment( $comment, $args, $depth ) {
		...
		echo farmart_comment_template( $comments );
...
if ( ! function_exists( 'farmart_comment_template' ) ) :
	function farmart_comment_template( $comments ) {
		...
		$date      = sprintf( esc_html__( '%1$s', 'farmart' ), $comments['comment_date'] );
		$output[]  = sprintf( '<a href="%s" class="date">%s</a>', esc_url( $comments['comment_link'] ), $date );
		$output[]  = '</div>';
		if ( $comments['comment_approved'] == '0' ) {
			$output[] = sprintf( '<em class="comment-awaiting-moderation">%s</em>', esc_html__( 'Your comment is awaiting moderation.', 'farmart' ) );
		} else {
		...
		if ( current_user_can( 'edit_comment', $comments['comment_ID'] ) ) {
			$output[] = sprintf( '<a class="comment-edit-link" href="%s">%s</a>', esc_url( admin_url( 'comment.php?action=editcomment&amp;c=' ) . $comments['comment_ID'] ), esc_html__( 'Edit', 'farmart' ) );
		}
