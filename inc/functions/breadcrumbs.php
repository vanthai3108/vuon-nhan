 ...
 */
function farmart_breadcrumbs( $args = '' ) {
	$args = wp_parse_args(
		$args, array(
			'separator'         => '',
			'home_class'        => 'home',
			'before'            => '<span class="before">' . esc_html__( 'You are here: ', 'farmart' ) . '</span>',
			'before_item'       => '',
			'after_item'        => '',
			'taxonomy'          => 'category',
			...
			'show_on_front'     => true,
			'labels'            => array(
				'home'      => esc_html__( 'Home', 'farmart' ),
				'archive'   => esc_html__( 'Archives', 'farmart' ),
				'blog'      => esc_html__( 'Blog', 'farmart' ),
				'search'    => esc_html__( 'Search results for', 'farmart' ),
				'not_found' => esc_html__( 'Not Found', 'farmart' ),
				'portfolio' => esc_html__( 'Portfolio', 'farmart' ),
				'author'    => esc_html__( 'Author:', 'farmart' ),
				'day'       => esc_html__( 'Daily:', 'farmart' ),
				'month'     => esc_html__( 'Monthly:', 'farmart' ),
				'year'      => esc_html__( 'Yearly:', 'farmart' ),
			),
		)
	);

	$args = apply_filters( 'farmart_breadcrumbs_args', $args );
	...
	if ( ! $args['home_class'] ) {
		...
		$items[] = sprintf(
			...
			$args['before_item'],
			$args['home_class'],
			apply_filters( 'farmart_breadcrumbs_home_url', get_home_url() ),
			$args['labels']['home'],
			$args['after_item']
		);
	...
	elseif ( is_single() ) {
		...
		if ( ! empty( $terms ) ) {
			...
			$terms   = farmart_get_term_parents( $term->term_id, $taxonomy );
			$terms[] = $term->term_id;
	...
	elseif ( is_page() ) {
		if ( ( function_exists( 'is_cart' ) && is_cart() ) || ( function_exists( 'is_checkout' ) && is_checkout() ) ) {
			...
			$pages = farmart_get_post_parents( get_queried_object_id() );
		...
		$terms        = farmart_get_term_parents( get_queried_object_id(), $current_term->taxonomy );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'farmart' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'farmart' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'farmart' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
 ...
 */
function farmart_get_term_parents( $term_id = '', $taxonomy = 'category' ) {
 ...
 */
function farmart_get_post_parents( $post_id = '' ) {
