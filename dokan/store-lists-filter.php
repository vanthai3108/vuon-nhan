...
<div id="dokan-store-listing-filter-wrap">
    ...
    <div class="farmart-header-filter">
        <div class="left">
            <p class="item store-count">
				<?php printf( '%1$s: %2$d', __( 'Total store showing', 'farmart' ), $number_of_store ); ?>
            </p>
        </div>
        ...
        <div class="right">
            <div class="item">
                ...
                <button class="dokan-store-list-filter-button dokan-btn dokan-btn-theme">
					<?php esc_html_e( 'Filter', 'farmart' ); ?>
                </button>
            </div>

            <form name="stores_sorting" class="sort-by item" method="get">
                <label><?php esc_html_e( 'Sort by', 'farmart' ); ?>:</label>

                <select name="stores_orderby" id="stores_orderby"
                        aria-label="<?php esc_attr_e( 'Sort by', 'farmart' ); ?>">
					<?php
            ...
            <div class="toggle-view item">
                <span class="dashicons dashicons-screenoptions" data-view="grid-view"><?php echo Farmart\Icon::get_svg( 'grid' ) ?></span>
                <span class="dashicons dashicons-menu-alt" data-view="list-view"><?php echo Farmart\Icon::get_svg( 'menu' ) ?></span>
            </div>
        </div>
    </div>
</div>
...
<form role="store-list-filter" name="dokan_store_lists_filter_form" id="dokan-store-listing-filter-form-wrap">
	...
	if ( apply_filters( 'dokan_load_store_lists_filter_search_bar', true ) ) : ?>
        <div class="store-search grid-item">
            <input type="search" class="store-search-input" name="dokan_seller_search"
                   placeholder="<?php esc_attr_e( 'Search Vendors', 'farmart' ); ?>">
        </div>
	<?php endif;
    ...
    <div class="apply-filter">
        <button id="cancel-filter-btn"
                class="dokan-btn dokan-btn-theme"><?php esc_html_e( 'Cancel', 'farmart' ); ?></button>
        <button id="apply-filter-btn" class="dokan-btn dokan-btn-theme"
                type="submit"><?php esc_html_e( 'Apply', 'farmart' ); ?></button>
    </div>
